...
contract Esusu {
    ...
    mapping (uint => GroupDonation[]) public groupDonations;

    // create a mapping between groups and groupmembers (name)
    mapping (string => GroupMember[]) public groupinfo;

    // create a mapping between groups and groupmembers (id)
    mapping (uint => GroupMember[]) public groupinfo_i;

    // mapping between group id and list of groupmembers
    mapping (uint => GroupMember[]) public groupMemberDict;

    // mapping between group index and group
    mapping (uint => Group) public groupDict;
    ...
    mapping (address => EsusuUser) public userProfile;
